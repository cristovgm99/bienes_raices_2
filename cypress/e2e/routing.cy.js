...
describe('Prueba la navegación y Routing del Header y Footer', () => {
    it('Prueba la navegación del Header', () => {
        ...
        cy.get('[data-cy="navegacion-header"]').find('a').eq(0).invoke('text').should('match', /^Nosotros/)

        cy.get('[data-cy="navegacion-header"]').find('a').eq(1).invoke('attr', 'href').should('equal', '/propiedades')
        cy.get('[data-cy="navegacion-header"]').find('a').eq(1).invoke('text').should('match', /^Anuncios/)

        cy.get('[data-cy="navegacion-header"]').find('a').eq(2).invoke('attr', 'href').should('equal', '/blog')
        cy.get('[data-cy="navegacion-header"]').find('a').eq(2).invoke('text').should('match', /^Blog/)

        cy.get('[data-cy="navegacion-header"]').find('a').eq(3).invoke('attr', 'href').should('equal', '/contacto')
        cy.get('[data-cy="navegacion-header"]').find('a').eq(3).invoke('text').should('match', /^Contacto/)
        
    })

    it('Prueba la navegación del Feader', () => {
        ...
        cy.get('[data-cy="navegacion-footer"]').find('a').eq(0).invoke('text').should('match', /^Nosotros/)

        cy.get('[data-cy="navegacion-footer"]').find('a').eq(1).invoke('attr', 'href').should('equal', '/propiedades')
        cy.get('[data-cy="navegacion-footer"]').find('a').eq(1).invoke('text').should('match', /^Anuncios/)

        cy.get('[data-cy="navegacion-footer"]').find('a').eq(2).invoke('attr', 'href').should('equal', '/blog')
        cy.get('[data-cy="navegacion-footer"]').find('a').eq(2).invoke('text').should('match', /^Blog/)

        cy.get('[data-cy="navegacion-footer"]').find('a').eq(3).invoke('attr', 'href').should('equal', '/contacto')
        cy.get('[data-cy="navegacion-footer"]').find('a').eq(3).invoke('text').should('match', /^Contacto/)
